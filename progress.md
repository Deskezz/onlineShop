...
## ЭТАП 12: Полировка (Завершён)
- [x] Усилены базовые SEO-метаданные в `src/app/[locale]/layout.tsx` (title template, Open Graph, Twitter card).
- [x] Добавлена локализованная страница `404` в `src/app/[locale]/not-found.tsx`.
- [x] Улучшены focus-visible состояния ссылок в `Footer`.
- [x] Улучшены a11y-лейблы для переключателей темы/языка и кнопки мобильного меню.
- [x] Добавлены i18n-ключи `common.openMenu` в `messages/ru.json` и `messages/en.json`.
- [x] Проверка сборки `npm run build` прошла успешно.
...
## Следующий шаг
Ожидаем дальнейшие команды пользователя.
